...
### 5. Import the pure-record characterization from Appendix D
...
in Appendix D.  In Lean this is the theorem `inducedPure_traceAxioms`; the
...
This is where the hard Appendix D result is fully used.  It is not replaced by
...
4. In the pure-trace lemma, invoke Appendix D for the ordinal mutual-information
   ...
   ordinal Appendix D comparison remains available at every constant payoff,
...
| Appendix D import | `PureTrace.lean`: `inducedPure_traceAxioms`, `inducedPure_MIRep`, `inducedPure_blockSameScale` |
...
kernel-checked Appendix D characterization and otherwise derives every
