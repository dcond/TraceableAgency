...
The full proof invokes the already checked pure-record theorem at a constant
payoff.  `inducedPure_traceAxioms_of_components` constructs every field of the
pure axiom bundle from the full main-text axioms:

| Pure-record field | Constructed from full axioms |
|---|---|
| weak order | A1 |
...
`TraceableAgency.provedMainCharacterizationWithMoreover` is a closed theorem.
...
mathematical declaration.  `PaperFaithfulAxiomAudit.lean` uses
...
`PaperFaithfulDependencyAudit.lean` recursively follows declaration types and
...
- verifies `CERTIFICATE_SHA256SUMS`;
- builds the exact `Theorem1Verification` target;
- rejects `sorry`, `admit`, declaration-level project axioms, bare `opaque`
  declarations, and named kernel-bypass primitives in both source trees;
- prints and type-checks the public theorem surface; and
- replays `Theorem1Verification.Proof` with `leanchecker --fresh`.
...
paper-line correspondence is in `CLAIM_MAP.md`; byte identities are in
`CERTIFICATE_SHA256SUMS`.
