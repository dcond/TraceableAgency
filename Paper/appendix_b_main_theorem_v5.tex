...
Suppose Axioms \textup{(A1)}, \textup{(A2)}, and
\textup{(A5)--(A7)} hold.
...
Let $o_\ast$ be the outcome fixed in Appendix~\ref{lf:app:pure-trace} to
witness Axiom~\textup{(A4)}.  We now use Proposition~\ref{pt:prop:main} on the
constant-$o_\ast$ lift
\[
K_P(o,r\mid a)=\1_{\{o=o_\ast\}}P(r\mid a).
\]
This lift commutes with block formation, record processing, action processing,
limits on each finite simplex, and compounding.  Thus Axiom~\textup{(A1)} gives
the pure weak order, while Lemma~\ref{lem:relevance-bridge} turns
Axiom~\textup{(A4)} into its positive orientation; Axiom \textup{(A2)} gives continuity; Axiom \textup{(A5)} gives
block coherence; Axioms \textup{(A6)} and \textup{(A7)} give the two data
processing conditions, including every null-row completion; and
Lemma~\ref{pt:lem:finite-branch-extension} gives finite-branch continuation.  These are precisely the
...
nontrivial action fibre and every experiment $E$ that pays $o_\ast$ surely satisfy
\begin{equation}
...
-W_q\bigl(C(P,o_\ast)\bigr)=m_y\bigl[u(o)-u(o_\ast)\bigr].
...
projection, insert it at branch $y$, and put the payoff $o_\ast$ for sure at
...
Support restriction and Corollary~\ref{pt:cor:one-branch-a8} give
\[
E\succeq_{\bar r_y}G
\quad\Longleftrightarrow\quad
J_y(E)\succeq_q J_y(G).
\]
Different continuation record alphabets are first put in a common disjoint union.  Moreover, $J_y$ commutes
...
Suppose first that $S_y$ has at least two elements.  At payoff $o_\ast$, compare
...
pay $o_\ast$ surely, so Lemma~\ref{lf:lem:trace-scale} and
...
Locally, deterministic payoffs $o$ and $o_\ast$ have values $u(o)$ and $u(o_\ast)$ by
...
\oplus\tfrac12 C(P,h[y\mapsto o_\ast])
\]
and
\[
\tfrac12 C(P,g[y\mapsto o_\ast])
\oplus\tfrac12 C(P,h[y\mapsto o])
\]
have the same marked terminal law.  Away from $y$ they contain the same two
payoff-posterior atoms; at $y$ they contain $o$ and $o_\ast$ with the same mass
...
$h$ to be the constant-$o_\ast$ profile and applying
Lemma~\ref{lf:lem:branch-increment}, then changing the finitely many branches
one at a time, gives
\begin{equation}
W_q(C(P,g))-W_q(C(P,o_\ast))
=\sum_y m_y\bigl[u(g(y))-u(o_\ast)\bigr].
\label{lf:eq:telescope}
\end{equation}
The constant-$o_\ast$ compound has exactly the information in the
first-stage record.  Lemma~\ref{lf:lem:trace-scale} therefore gives
$W_q(C(P,o_\ast))=u(o_\ast)+\lambda I_q(A;Y)$.  Substitute this into
\eqref{lf:eq:telescope} and use $\sum_y m_y=1$.
\end{proof}
...
coordinate.  Hence Axiom~\textup{(A6)} gives
...
\end{equation}
with the same $u$ and $\lambda$.  Axiom~\textup{(A5)}, first by duplication and
...
label, which changes neither term of $V$; this proves Axiom~\textup{(A5)} and
...
Axiom~\textup{(A6)}.  Under an action processor, equation
\eqref{eq:action-processing} again leaves the payoff-record marginal unchanged,
and the joint law forms the Markov chain
\[
(O,R)-A-B.
\]
Thus $I(B;O,R)\le I(A;O,R)$, proving Axiom~\textup{(A7)}.  This argument uses
the undivided joint-law equation and so also covers every completion on a null
processed action.

For the binary compounds in Axiom~\textup{(A8)}, the expected-utility and mutual-information chain rules give
\begin{align*}
&V\bigl(q,P\triangleright(K,M)\bigr)-V\bigl(q,P\triangleright(L,M)\bigr)\\
&\qquad =\left(\sum_a q(a)P(1\mid a)\right)
 \bigl[V(q_1,K)-V(q_1,L)\bigr].
\end{align*}
The first-stage information term and the continuation following record $2$ cancel.  Because record $1$ has
positive probability under $q$, the coefficient is positive, so the two differences have the same sign.  This
proves the weak biconditional in Axiom~\textup{(A8)} and its strict form.

Because $u$ is nonconstant, choose $o^+,o^-$ with $u(o^+)>u(o^-)$.  In the
two-action channel of Axiom~\textup{(A3)}, its two pure lotteries have these
values and zero information, so Axiom~\textup{(A3)} holds.  For any $o\in O$,
form the four-action channel described in Axiom~\textup{(A4)}.  Its two
lotteries pay $o$ surely and have information values $\log2$ and zero.  Since
$\lambda>0$, the required strict comparison holds---indeed, it holds at every
$o$, although Axiom~\textup{(A4)} requires only one.  The same block calculation used above gives the moreover
