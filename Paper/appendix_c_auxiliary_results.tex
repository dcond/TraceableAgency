...
$\E_{qK}[u(O)]=\E_{qL}[u(O)]$ and $\mu_{qK}=\mu_{qL}$, then $(q,K)\sim(q,L)$.
\end{corollary}
...
\[
 ...
 =V(p,L)-V(q,K).
\]
When the material parts agree, $\E_{qK}[u(O)]=\E_{pL}[u(O)]$, the left-hand side equals
...
constant, contradicting Axiom~\textup{(A7)} for family 2.  (The case is not idle: an index with constant $u_2$,
...
$\overline u>\underline u$ by Axiom~\textup{(A7)}.  Suppose $\lambda_2<\lambda_1$ and fix
