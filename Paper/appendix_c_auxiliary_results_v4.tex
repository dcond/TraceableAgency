...
improvements aggregate.  Block coherence holds.  For Axiom~\textup{(A3)}, choose the two outcomes so that
...
\[
 ...
 =\sum_y m(y)(\Delta_y+\varepsilon).
\]
Thus Axiom~\textup{(A8)}, including strictness, holds; the remaining axioms are immediate.  Duplication coherence fails.
The two fixed relevance tests are untagged, so their scores reduce to $U+I$ and both strict comparisons hold.
...
The strict clause of Axiom~\textup{(A8)} fails.
\end{proof}

\subsection{Proofs of the choice implications}\label{app:choice-proofs}

Results stated in the text only in summary form (Propositions~\ref{prop:binary}, \ref{prop:wtp},
...
\[
 I_{qK}(A;Z)=\sum_aq(a)\KL\!\left(K_Z(\cdot\mid a)\,\middle\|\,m_{qK}\right),
\]
...
\]
If $q_0$ and $q_1$ are optimal with $m_{q_0K}\ne m_{q_1K}$, strict concavity of Shannon entropy and linearity of
$q\mapsto m_{qK}$ and of the remaining sum give $V(tq_0+(1-t)q_1,K)>C^*(K)$ for $t\in(0,1)$, a contradiction.
...
$\supp(q^*)\subseteq\mathcal A^*$.  Conversely, if $m_{qK}=m^*$ and $\supp(q)\subseteq\mathcal A^*$, then
\[
 V(q,K)
 =\sum_aq(a)\bigl[\bar u(a)+\lambda\KL(K_a\|m^*)\bigr]
 =C^*(K).
\]
Affine independence makes the representation $m^*=\sum_aq(a)K_a$ with $\supp(q)\subseteq\mathcal A^*$ unique.
Identical rows share $\bar u(a)$ and $\Sh(K_a)$, so both $m_{qK}$ and the displayed expression for $V$ are
...
$m_{q_xK}=(1-x)K_a+xK_b=:m_x$, and for $x\in(0,1)$ define
\[
...
For \textup{(c)}$\Rightarrow$\textup{(a)}: by the identity in the proof of Corollary~\ref{cor:matching}, the
...
We restate Proposition~\ref{prop:comparative} with its standing hypotheses.  Both families are defined over the
same outcome set $O$, on the common domain of all finite pairs, with mutual information in one fixed base.  A
