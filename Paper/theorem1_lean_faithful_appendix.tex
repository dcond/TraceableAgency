% Appendix A of the Lean-faithful supplement.  The preview driver appends the
% self-contained pure-trace statement and proof from trace_tempered_choice_v3
% as Appendix B, so the two documents share one source for that long proof.

\section{Lean-faithful proof of Theorem 1}\label{app:lean-faithful-main}

The proof has one substantial auxiliary input: the pure-trace proposition
stated next and proved from the primitive pure-record conditions in
Appendix~\ref{lf:app:pure-trace}.  Everything after that input is the calibrated
assembly used by the checked Lean proof.  We also use the ordinary
mixture-space theorem: a complete and transitive mixture order that satisfies
independence and admits calibration on every closed mixture segment has an
...
&&
\boxed{\substack{\text{pure-trace proposition (Appendix B) and}\\
...
\subsection{The auxiliary pure-trace proposition}

Appendix~\ref{lf:app:pure-trace} defines the pure-record domain, the block
comparison notation, Conditions \textup{(P1)--(P5)}, and the mutual information
$I_{q,P}(A;X)$.  The exact input is stated here for use before it is proved;
Appendix~\ref{lf:app:pure-trace} states it formally as
Proposition~\ref{pt:prop:main}.  Thus there is one auxiliary proposition, not
two separately numbered copies, and Theorem~1 remains the unique theorem of
the joint payoff--record model.

\begin{quote}
\noindent\textbf{Pure-trace proposition.}
For a family $\{\succeq_P\}_P$ on all finite record channels described in
Appendix~\ref{lf:app:pure-trace}, the following are equivalent.
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
\item The family satisfies Conditions \textup{(P1)--(P5)}.
\item For every pair of finite sets $A,X$, every channel
  $P:A\to\D(X)$, and all $q,q'\in\D(A)$,
  \[
  q\succeq_Pq'
  ...
  \]
\end{enumerate}
Moreover, on every finite block environment
$\bigsqcup_{k\in K}P_k$, all block-supported cross-channel comparisons use
that same information scale: for distinct $i,j\in K$ and all
$q_i\in\D(A_i)$, $q_j\in\D(A_j)$,
\[
...
Appendix~\ref{lf:app:pure-trace} proves both directions and the moreover
clause.  Appendix~A uses only \textup{(i)$\Rightarrow$(ii)} and its common-scale
clause, applied to the constant-payoff lift.
...
and
\[
\sum_{o,r}p_{qK}(o,r)u(o)=\E_{q,K}[u(O)],
\]
respectively.  Together with sequentialisation, they give
\begin{equation}
W_q(K)=\E_{q,K}[u(O)]+\lambda I_{qK}(A;O,R).
...
\begin{align*}
\E[u(O)]
  &=\sum_{y:m(y)>0} m(y)\E_{q_y,K^y}[u(O)],\\
I(A;Y,O,R)
  &=I(A;Y)+\sum_{y:m(y)>0} m(y)I_{q_y,K^y}(A;O,R).
\end{align*}
