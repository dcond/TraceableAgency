% Body fragment.  It can be inserted into trace_tempered_choice_v3.tex after
% \appendix, or compiled through theorem1_lean_faithful_appendix_preview.tex.

\section{Proof of Theorem 1}\label{app:lean-faithful-main}

The only substantial result imported from elsewhere in the paper is the
pure-record characterisation in Appendix D.  We also use the ordinary
...
affine representation.  The proof identifies the value of changing one
deterministic terminal payoff and then telescopes.  An arbitrary joint channel
is reduced to this case by sequentialisation.
...
We now use the pure-record theorem proved in Appendix D.  Apply it to the
...
hypotheses of the pure-record theorem.  Its conclusion is that every
...
affine under public mixtures.  The pure-record theorem says that $W_q$ and
