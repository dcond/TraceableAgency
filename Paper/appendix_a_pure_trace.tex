...
Fix an outcome $o_0\in O$.  Every channel between nonempty finite sets
$P:A\to\D(X)$ has the constant-payoff lift
\begin{equation}
K_P(o,x\mid a):=\1_{\{o=o_0\}}P(x\mid a).
...
processing, and a compound $P_1\triangleright\{Q^x\}$ are shorthand for the
...
neutral by Axiom~\textup{(A4)} applied in both directions.  No new primitive
relation or behavioural condition is introduced here.

Two pieces of notation will be used repeatedly.  If
$S:A\to\Delta(B)$ is an action report, let $S^qP:B\to\Delta(X)$ denote any
...
\[
m_{q,P}(x):=\sum_a q(a)P(x\mid a),
\qquad
r_x^{q,P}(a):=\frac{q(a)P(x\mid a)}{m_{q,P}(x)}
\quad\text{when }m_{q,P}(x)>0,
\]
...
\[
I_{q,P}(A;X)
:=\Sh(m_{q,P})-\sum_a q(a)\Sh(P(\cdot\mid a))
=\Sh(q)-\int_{\D(A)}\Sh(r)\,d\mu_{q,P}(r).
\]

\begin{proposition}[Pure-trace lemma]\label{pt:prop:main}
Suppose the preference family satisfies main-text Axioms
\textup{(A1)--(A6)} and \textup{(A8)}.  Then, for every record channel between
...
processing, branchwise continuation, or positive trace orientation is an
appeal to Axiom \textup{(A1)}, \textup{(A2)}, \textup{(A3)}, \textup{(A4)},
\textup{(A5)}, \textup{(A6)}, or \textup{(A8)}, respectively, on the
...
spine may use their lemma statements as interfaces.  Product calibration is
...
\subsubsection{Stage 1: posterior-law quotient and derived continuity}
...
Assume Axioms~\textup{(A1)} and \textup{(A3)}.
...
\textup{(b)}  Under Axiom~\textup{(A2)}, the comparisons in \textup{(a)} are also closed in the channels at fixed alphabets
...
\]
Fixedness of the alphabets is inherited from Axiom~\textup{(A2)}, which is stated at a fixed pair of alphabets.
\end{lemma}

\begin{proof}
\textup{(a)}  Completeness follows from Axiom~\textup{(A1)} applied to the two-block environment $P_1\sqcup P_2$: either
$q_1^0\succeq_{P_1\sqcup P_2}q_2^1$ or $q_2^1\succeq_{P_1\sqcup P_2}q_1^0$.  In the second case, applying
Axiom~\textup{(A3)} to the two-block environment with blocks $P_1$ and $P_2$, using the ordered pair $(i,j)=(1,0)$,
...
Axiom~\textup{(A3)} with block pair $(1,2)$, the first hypothesis is equivalent to
\[
...
\]
Transitivity of the weak order given by Axiom~\textup{(A1)} in this common environment gives
\[
q_1^1\succeq_{\bigsqcup_{k=1}^3 P_k}q_3^3,
\]
and applying Axiom~\textup{(A3)} again, now deleting the irrelevant middle block, yields
...
the block-supported lotteries $q_1^0,q_2^1$ are fixed, Axiom~\textup{(A2)} applied at this pair of alphabets gives
\[
q_1^0\succeq_{P\sqcup Q}q_2^1.
\qedhere
\]
...
\begin{proof}
By Lemma~\ref{pt:lem:blackwell}, there are stochastic kernels $T$ and $T'$ such that $Q=PT$ and $P=QT'$.  Axiom~\textup{(A4)} gives
\[
...
\]
By the reverse-block equivalence in Lemma~\ref{pt:lem:blockcoh} (equivalently, Axiom~\textup{(A3)} applied to
...
Assume Axioms~\textup{(A1)}, \textup{(A3)}, \textup{(A4)}, and
\textup{(A6)}.  Fix a finite action set $A$, a
...
Axiom~\textup{(A6)} requires the two continuation channels on a given branch to share a record alphabet, whereas $P$
...
primitive closedness of Axiom~\textup{(A2)} has been used, through Lemma~\ref{pt:lem:blockcoh}.
...
Axiom~\textup{(A3)} and Axiom~\textup{(A4)} impose, with the convex order supplying compactness where record alphabets
grow without bound.  Only weak preference is closed in the limit; no strictness claim is made or used.

\subsubsection{Stage 2: affine fibres and canonical boundary behaviour}
...
information implements $\delta_q$.  Axiom~\textup{(A8)}, transported through
the quotient comparison, gives
\[
...
pushforward satisfies $S^qP=P\pi^{-1}$ (explicitly: $(S^qP)(x\mid a')=P(x\mid\pi^{-1}(a'))$).  Axiom~\textup{(A5)} gives
\[
...
\]
By the irrelevant-blocks clause of Axiom~\textup{(A3)}, block comparisons depend only on the pair of channels involved, not on
...
For a nonempty $B\subseteq A$ and a finite law $\mu$ whose barycentre is supported on $B$, coordinatewise non-negativity of probabilities implies that
...
\emph{Forward direction (Axiom~\textup{(A5)} projection).}  Let $S:A\to B$ map all $a\notin B$ to some fixed $b_0\in B$.  The coarsened
...
\emph{Reverse direction (Axiom~\textup{(A5)} inclusion with completion).}  Let $I:B\hookrightarrow A$ be the inclusion.  Then
$(r|_B)I=r$.  The Bayesian pushforward $I^{r|_B}(P|_B)$ is pinned down on $B$ and arbitrary on $A\setminus B$.  Choose
the Axiom~\textup{(A5)} completion to be any extension $\widetilde P:A\to\Delta(X)$ of $P|_B$.  Then
\[
(r|_B,P|_B)\succeq(r,\widetilde P).
\]
...
\subsubsection{Stage 3: product transport and the cross-prior bridge}
...
Assume Axioms~\textup{(A1)} and \textup{(A3)--(A6)}.  Let $A_1,A_2$ be finite action sets and let
...
Axiom~\textup{(A6)} requires the two continuation channels used on a given branch to share a record alphabet.  If
...
(Axiom~\textup{(A3)}, Lemma~\ref{pt:lem:blockcoh}).  The two indifferences and transitivity of the weak order in that
...
$r_{x_2}^0\succeq_{\widetilde P_1\sqcup\widetilde Q_1}r_{x_2}^1$ of Axiom~\textup{(A6)} holds for every branch with
$m(x_2)>0$, with the common first stage $\widetilde R_2$ and the two constant continuation profiles.
Axiom~\textup{(A6)} yields~\eqref{pt:eq:bgcompound}, hence the left-hand side of~\eqref{pt:eq:backgroundinert} by Step~1.
...
F_p(\mu)\ge F_p(\mu').
\]
The symmetric clause of Lemma~\ref{pt:lem:background} identifies every second-coordinate slice with the $F_r$-order.  Axiom~\textup{(A8)}, the
...
It remains to transfer the product-lifted comparison back to the original two
blocks.  Let $P^\uparrow:A\to\Delta(X\times\{*\})$ be the singleton-record
extension $P^\uparrow((x,*)\mid a)=P(x\mid a)$.  Axiom~\textup{(A5)} applied
to the projection $\pi_A:A\times B\to A$ gives
\[
(q\otimes r,P\otimes U_B)\succeq(q,P^\uparrow).
\]
The record bijection $X\times\{*\}\to X$ and its inverse, followed by
Axiom~\textup{(A4)} in both directions, identify $(q,P^\uparrow)$ with
$(q,P)$.  Thus the displayed comparison also holds with $(q,P)$ on the
right.  Conversely, let $T:A\to\Delta(A\times B)$ be
\[
T((a,b)\mid a')=\1_{\{a=a'\}}r(b).
\]
Then $qT=q\otimes r$, and the Bayesian pushforward $T^qP^\uparrow$ is exactly
$P\otimes U_B$.  Axiom~\textup{(A5)} therefore gives the reverse comparison.
Hence
...
\subsubsection{Stage 4: branch coefficients, cocycle, and support faces}
...
alphabets if needed, Axiom~\textup{(A6)} compares the two continuation profiles that differ only in branch $\bar x$.
...
Since $|\operatorname{supp}(r)|\ge2$, Axiom~\textup{(A8)} on the support face and Lemmas~\ref{pt:lem:supprestrict} and~\ref{pt:lem:convnorm} imply $F_r$ is nonconstant.  The continuous affine functions $g$ and $F_r$ are therefore nonconstant
...
\]
Since $|B|\ge2$ and $r|_B$ is full support on $B$, Axiom~\textup{(A8)} and Lemma~\ref{pt:lem:convnorm} give
\[
...
Axiom~\textup{(A6)} applied to the reversed comparison gives $L_q(\nu'-\nu)>0$, i.e.\ $L_q(\nu-\nu')<0$.  If $L_r(\nu-\nu')=0$, then
$\nu\sim_r\nu'$, so both $\nu\succeq_r\nu'$ and $\nu'\succeq_r\nu$; applying Axiom~\textup{(A6)} weakly in both directions gives
$L_q(\nu-\nu')\ge 0$ and $L_q(\nu-\nu')\le 0$, hence $L_q(\nu-\nu')=0$.
...
for some $c>0$.)  Since $r$ has full support and Axiom~\textup{(A8)} gives $F_r(\chi_r)>F_r(\delta_r)=0$, we have
...
\subsubsection{Stage 5: compatibility collapse and entropy identification}
...
Z(q):=1+\kappa H(q).
\]
For a nondegenerate prior, Axiom~\textup{(A8)} gives $H(q)>0$, and~\eqref{pt:eq:QAslope} gives
\begin{equation}
Z(q)>0.
...
Thus $Z(q)=1$ and $\kappa H(q)=0$ for every $q$.  Axiom~\textup{(A8)} gives a nondegenerate $q$ with $H(q)>0$, hence
...
\]
both closed.  Now fix $c\ge0$.  By Axiom~\textup{(A8)}, there exists a binary prior $\theta=(s,1-s)$ with
$h_\theta:=H(\theta)>0$.  Choose $n\in\mathbb{N}$ and $\lambda\in[0,1]$ such that $c=\lambda n h_\theta$
(take $n$ large enough that $c\le n h_\theta$, then set $\lambda:=c/(n h_\theta)$).

Let $w_n:=\theta^{\otimes n}$ be the $n$-fold product prior on $B_n:=\{0,1\}^n$.  By product additivity,
$H(w_n)=nH(\theta)=n h_\theta$.  Define the erasure experiment $E_{n,\lambda}:B_n\to\Delta(\{*,1,\dots,2^n\})$ which
...
\[
\widehat F_{w_n}(\mu_{w_n,E_{n,\lambda}})=\lambda H(w_n)=\lambda n h_\theta=c.
\]
...
Positive trace orientation (Axiom~\textup{(A8)}) forces $\alpha>0$.
...
Fix $P:A\to\Delta(X)$ and $q,q'\in\Delta(A)$.  By the duplication clause of Axiom~\textup{(A3)},
\[
q\succeq_P q'
\quad\Longleftrightarrow\quad
...
The within-channel equivalence is Lemma~\ref{pt:lem:globalsketch}.  For the
block clause, apply the same common-scale bridge to the fixed environment
$\bigsqcup_{k\in J}P_k$.  A lottery supported on block $i$ has a constant
block label, so its mutual information is
\[
