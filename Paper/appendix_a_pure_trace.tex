...
laws with prior $q$, and $\mu_{q,P}\in\mathcal M_q$ for every finite channel
$P$.
...
$P_1\sqcup P_2\sqcup P_3$.  Axiom~\textup{(A3)} transports the two
hypotheses to that environment, Axiom~\textup{(A1)} composes them there, and
Axiom~\textup{(A3)} deletes the middle block.  Applying Axiom~\textup{(A3)}
...
the duplication clause of Axiom~\textup{(A3)}.  Finally, a block channel is a
...
\[
 X_s:=\{x:m_P(x)>0,\ r_x^{q,P}=s\},
 \qquad
 Y_s:=\{y:m_Q(y)>0,\ r_y^{q,Q}=s\}.
\]
...
Axiom~\textup{(A4)} applied to $T$ and $T'$ yields the two weak comparisons;
...
\end{equation}
defines a channel and $\mu_{q,C_\mu}=\mu$.
...
Let $C:A\to\Delta(\{0,1\})$ be the uninformative first-stage channel with
$C(0\mid a)=t$ for every $a$.  Both branches are reached, and their posterior
is $q$.  The compounds
\[
 C\triangleright\{P,R\},
 \qquad
...
mixtures only by record labels.  If $(q,P)\succeq(q,Q)$, Axiom~\textup{(A6)}
applied to branch $0$, together with reflexivity on the common branch $1$,
gives the aggregate comparison.

Conversely, suppose the aggregate comparison holds but
$(q,P)\not\succeq(q,Q)$.  Completeness gives $(q,Q)\succ(q,P)$.  Apply the
strict clause of Axiom~\textup{(A6)} with the two continuation profiles
reversed.  Branch $0$ is reached with probability $t>0$, so it yields the
strict reverse aggregate comparison, contradicting the assumed weak one.
Posterior-law substitution transfers both conclusions between the compounds
and the tagged mixtures.
\end{proof}
...
$F_q$.  When $|A|\ge2$, Axiom~\textup{(A8)} makes the order nonconstant, so
...
deleting its record.  Axiom~\textup{(A4)} and
\eqref{pt:eq:fibre-representation} therefore imply
\[
 F_q(\mu_{q,P})\ge F_q(\delta_q).
\]
...
If $|A|\ge2$, Axiom~\textup{(A8)} says that full revelation is strictly
...
\begin{proof}
\emph{Part (a).}
Regard $\pi$ as a deterministic action-report kernel.  Axiom~\textup{(A5)}
gives
\[
...
direction of \eqref{pt:eq:undetectable} is Axiom~\textup{(A5)}.  For the
...
it back through $R$ recovers $P$.  Axiom~\textup{(A5)} applied to
...
\[
 ...
 S^qP=P_B.
\]
Axiom~\textup{(A5)} gives $(q,P)\succeq(q_B,P_B)$.  Conversely, let
$\iota:B\hookrightarrow A$ be the inclusion.  Then $q_B\iota=q$.  The exact
Bayesian completion equation pins down the reported channel on $B$ and leaves
all zero-mass rows in $A\setminus B$ arbitrary.  The all-completions clause of
Axiom~\textup{(A5)} therefore permits us to choose the completion to be the
...
stages derive product and branch scale coherence, with
Axiom~\textup{(A6)} providing the essential cardinal alignment across reached
branches.
...
$p$.  If $P\succeq_pQ$, the weak clause of Axiom~\textup{(A6)} applied to
...
The strict clause of Axiom~\textup{(A6)} therefore gives
...
Bayes' rule gives
\begin{equation}
 \mu_{p\otimes r,P\otimes R}=\mu_{p,P}\otimes\mu_{r,R}.
...
\[
 G_{q\otimes r}(\mu_{q,P}\otimes\delta_r)=G_q(\mu_{q,P}),
 \qquad
...
$A\times B\to A$ and Axiom~\textup{(A5)} give
...
$P\otimes U_B$; Axiom~\textup{(A5)} gives the reverse comparison.  Thus the
...
reaches $r$.  If a first-stage channel $P_1$ has reached branches of masses
$m_x$ and posteriors $r_x$, then every continuation profile satisfies
\begin{equation}
 G_q(\mu_{q,P_1\triangleright\{Q^x\}})
 =G_q(\mu_{q,P_1})+
 \sum_{x:m_x>0}m_x\beta_A(q,r_x)
 G_{r_x}(\mu_{r_x,Q^x}).
\tag{BA}\label{pt:eq:branchaggregation}
\end{equation}
For singleton $r_x$, the continuation value is zero and the corresponding
...
$\varepsilon i_B(\nu-\nu')$.  Axiom~\textup{(A6)}, support transport, and the
strict clause give all three signs:
\begin{align*}
 L_{r_B}^B(\nu-\nu')>0&\Longrightarrow
     L_q^A i_B(\nu-\nu')>0,\\
 L_{r_B}^B(\nu-\nu')<0&\Longrightarrow
...
the third, indifference supplies both weak branch comparisons, and the weak
part of Axiom~\textup{(A6)} supplies both aggregate inequalities.  By
...
For the aggregation formula, put $B_x:=\operatorname{supp}(r_x)$ and
\[
 \nu_x:=\pi_{B_x}\mu_{r_x,Q^x}\in\mathcal M_{(r_x)_{B_x}}.
\]
The compound law differs from the first-stage law by
\[
 \mu_{q,P_1\triangleright\{Q^x\}}-\mu_{q,P_1}
 =\sum_{x:m_x>0}m_x\,i_{B_x}
   (\nu_x-\delta_{(r_x)_{B_x}}).
\]
Apply $L_q^A$, use~\eqref{pt:eq:branch-linear}, and use
$G_{r_x}(\delta_{r_x})=0$.  Singleton summands vanish.  This is
...
\begin{equation}
 V_q(\mu_{q,P_1\triangleright\{Q^x\}})
 =V_q(\mu_{q,P_1})
  +\sum_{x:m_x>0}m_xV_{r_x}(\mu_{r_x,Q^x}).
...
becomes
\[
 \frac1{a_q}\,\beta_A(q,r_x)G_{r_x}
 =\frac1{a_{r_x}}G_{r_x}=V_{r_x}.
\]
...
\begin{align}
 ...
 V_q(\mu_{q,P_1\triangleright\{Q^x\}})
   &=V_q(\mu_{q,P_1})+\sum_{x:m_x>0}m_x
     V_{r_x}(\mu_{r_x,Q^x}).\tag{CR$'$}\label{pt:eq:final-chain}
\end{align}
...
\]
For nondegenerate $q$, Axiom~\textup{(A8)} and zero normalisation give
...
Axiom~\textup{(A8)} supplies a nondegenerate $q$ with $H_G(q)>0$, so
...
\begin{equation}
 V_q(\mu_{q,P})
 =\mathcal H(q)-\sum_{x:m_{q,P}(x)>0}
    m_{q,P}(x)\mathcal H(r_x^{\,q,P}).
...
\[
 \mathcal H(q)=V_q(\mu_{q,P})
 +\sum_{x:m_{q,P}(x)>0}m_{q,P}(x)\mathcal H(r_x),
\]
...
Axiom~\textup{(A8)}.  Choose $n\ge1$ and $\lambda\in[0,1]$ so that
\[
...
\[
 \mathcal H(q)=\alpha\Sh(q)
\]
for some $\alpha\ge0$.  Axiom~\textup{(A8)} forces $\alpha>0$.
Substitution in~\eqref{pt:eq:entropy-reduction} gives
\[
 V_q(\mu_{q,P})
 =\alpha\left(\Sh(q)-\sum_x m_{q,P}(x)
                    \Sh(r_x^{\,q,P})\right)
 =\alpha I_{q,P}(A;X),
\]
