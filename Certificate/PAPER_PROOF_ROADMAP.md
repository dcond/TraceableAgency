...
| Apply the grouping recursion and the proved finite Faddeev theorem to obtain entropy reduction and mutual information | `MIRep_of_paperInteractionCollapse`; `pureTraceRepresentation_of_conditions` |
...
`inducedPureConditions_of_components`; it takes exactly A1--A6 and A8, with
...
`provedPureTraceCharacterization`.  Thus, on every fixed-prior fibre,
...
| Appendix A pure-trace input | `PureTrace.lean`: `inducedPureConditions_of_components`, `inducedPureRepresentation_of_components`, `inducedPureBlocks_of_components` |
