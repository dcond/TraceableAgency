...
face-defect cocycle. -/
theorem MIRep_of_TraceAxioms_paperReduction_of_faceDefect
...
theorem MIRep_of_TraceAxioms_paperReduction
    (hfad : ClassicalFaddeevTheoremAssumptions.{u})
    {F : PrefFamily.{u}} (hax : TraceAxioms F) :
    MIRep F :=
  MIRep_of_TraceAxioms_paperReduction_of_faceDefect hfad hax
    (paperCardinalFaceDefectCocycle F hax)

/-- Sufficiency packaged in the public statement used by the main theorem. -/
theorem SufficiencyStatement_of_paperReduction
    (hfad : ClassicalFaddeevTheoremAssumptions.{u}) :
    SufficiencyStatement.{u} := by
  intro F hax
  exact MIRep_of_TraceAxioms_paperReduction hfad hax
