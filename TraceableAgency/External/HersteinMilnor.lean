...
-/
import Mathlib
import TraceableAgency.External.PosteriorApproximation
...
theorem posteriorLawHMRel_complete_of_axioms
  ...
  · left
    have hblock :=
      (hax.a3.finite_block (K := HMTwoBlock) (Act := Act) (Out := Out) (P := C)
...
theorem posteriorLawHMRel_transitive_of_axioms
  ...
  have hxy : F.rel commonP x y := by
    have hcommon :=
      (hax.a3.finite_block (K := HMTripleBlock) (Act := Act) (Out := Out) (P := C)
...
theorem posteriorLawHMRel_public_mix_independence_of_axioms
  ...
  · intro hmix
    ...
    have hleftPositive : BranchPositive C q (Sum.inl default) := by
      ...
      exact ht0
    have hstrictComp :=
      hax.a6.2 O₂ q C QG QE hbranch_rev
