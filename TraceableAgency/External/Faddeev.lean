...
4. **A3 block equivalence**: proved directly from `TraceAxioms.a3`

## References

* empowerment_v5.tex, Lemma faddeevsketch (lines 2560-2646)
...
/--
**A3 Duplicate-Block Equivalence from TraceAxioms**
...
Paper-specific bridge: TraceAxioms plus the constructed entropy-reduction
...
theorem coarseReveal_rel_id_of_A5
  ...
  have h :=
    hax.a5 (coarseRevealChannel Act) (sigmaDist p q)
...
theorem id_rel_coarseReveal_of_A5
  ...
  have h :=
    hax.a5 (Channel.idChannel : Channel K K) p
...
theorem alpha_strict_pos_of_positive_Hfun_witness
    (F : PrefFamily.{u}) (_hax : TraceAxioms F)
    (hcross : CrossPriorBlockRepresentation F)
...
Given the split Faddeev assumptions, TraceAxioms, and cross-prior block
...
Paper: Full sufficiency proof (lines 810-2646).
-/
...
Given all sufficiency external assumptions and TraceAxioms, derive the
...
Given TraceAxioms and the split Faddeev external assumptions, provides the
...
/-!
## Bundle of All Sufficiency External Assumptions

For convenience, bundle the remaining external assumptions for sufficiency.
-/

/--
**Sufficiency External Assumptions Bundle**

Bundles all remaining external assumptions needed for the full sufficiency proof:
1. Finite Blackwell same-posterior-law mutual garbling
2. Herstein-Milnor (posterior value representation)
3. Branch aggregation
4. Scale coherence
5. Cross-prior block bridge
6. Entropy regularity
7. Hfun block-embedding invariance
8. Coarse-reveal entropy reduction
9. Cardinal support boundary extension
10. Classical Faddeev theorem
This provides a single assumption bundle for stating the full sufficiency theorem.
-/
structure SufficiencyExternalAssumptions.{v} where
  blackwell : FiniteSamePosteriorLawBlackwellEquivalenceAssumptions.{v}
  herstein_milnor : FiniteHersteinMilnorAssumptions.{v}
  branch : FiniteBranchAggregationAssumptions.{v}
  scale : FiniteScaleCoherenceAssumptions.{v}
  cross_prior : FiniteCrossPriorBlockAssumptions.{v}
  entropy_regularity : FiniteEntropyRegularityFromAxiomsAssumptions.{v}
  hfun_block_embedding : FiniteHfunBlockEmbeddingInvarianceAssumptions.{v}
  coarse_reveal_entropy_reduction : FiniteCoarseRevealEntropyReductionAssumptions.{v}
  cardinal_support_boundary : FiniteCardinalSupportBoundaryAssumptions.{v}
  classical_faddeev : ClassicalFaddeevTheoremAssumptions.{v}

/-- Parallel sufficiency-level branch input using the faithful branch route.

This does not replace `SufficiencyExternalAssumptions.branch` yet; it exposes
the branch aggregation/chain-rule part without the legacy
`FiniteBranchAggregationAssumptions` monolith. -/
structure SufficiencyFaithfulBranchAssumptions.{v} where
  branch : FiniteFaithfulBranchAggregationAssumptions.{v}

/-- Branch aggregation output from the faithful sufficiency-level branch
bundle. -/
noncomputable def SufficiencyFaithfulBranchAssumptions.branchAggregationStructure
    (h : SufficiencyFaithfulBranchAssumptions.{u})
    (F : PrefFamily.{u}) (hax : TraceAxioms F)
    (hV : PosteriorValueRepresentation F) :
    BranchAggregationStructure F :=
  branchAggregationStructure_of_faithfulAssumptions h.branch F hax hV

/-- Branch chain output from the faithful sufficiency-level branch bundle. -/
noncomputable def SufficiencyFaithfulBranchAssumptions.branchChainStructure
    (h : SufficiencyFaithfulBranchAssumptions.{u})
    (F : PrefFamily.{u}) (hax : TraceAxioms F)
    (hV : PosteriorValueRepresentation F) :
    BranchChainStructure F :=
  branchChainStructure_of_faithfulAssumptions h.branch F hax hV

/-- Normalized chain rule exposed from the faithful sufficiency-level branch
bundle. -/
theorem SufficiencyFaithfulBranchAssumptions.normalizedChainRule
    (h : SufficiencyFaithfulBranchAssumptions.{u})
    (F : PrefFamily.{u}) (hax : TraceAxioms F)
    (hV : PosteriorValueRepresentation F)
    ...
    branchNormalizedValue (h.branchChainStructure F hax hV) q (P ▷ Q)
    =
    branchNormalizedValue (h.branchChainStructure F hax hV) q P
      + ∑ o : O,
          (Channel.outcomeMarginal P q) o *
          branchNormalizedValue
            (h.branchChainStructure F hax hV)
            (Channel.posterior P q o) (Q o) :=
  normalizedChainRule_of_faithfulAssumptions h.branch F hax hV q hq P Q

/-- Compatibility accessor: reconstruct the old posterior-replacement package
from the finite mutual-garbling Blackwell theorem plus internal A4/A3/A1
replacement plumbing. -/
theorem SufficiencyExternalAssumptions.blackwell_replacement
    (h : SufficiencyExternalAssumptions.{u}) :
    FiniteBlackwellPosteriorAssumptions.{u} :=
  blackwellPosteriorReplacement_of_samePosteriorGarblings h.blackwell

/--
**Full Sufficiency Statement from External Assumptions**

Given the bundle of all external assumptions, derive the full sufficiency statement:
TraceAxioms F → MIRep F.

This composes all sufficiency bridges. The full-support block/cross-support MI
comparison used by boundary extension is derived from the Faddeev form. The
one-sided A5 support comparisons are proved in `External.SupportRestriction`;
their A1/A3 common-block assembly is also proved in `External.SupportRestriction`.
-/
theorem SufficiencyStatement_of_external_assumptions
    (h : SufficiencyExternalAssumptions.{u}) :
    SufficiencyStatement.{u} := by
  intro F hax
  let hblackwell := h.blackwell_replacement
  have hfadStruct : FaddeevEntropyForm F :=
    faddeevEntropyForm_of_axioms F hblackwell h.herstein_milnor h.branch
      h.scale h.cross_prior h.entropy_regularity h.hfun_block_embedding
      h.coarse_reveal_entropy_reduction h.cardinal_support_boundary
      h.classical_faddeev hax
  have hfull : FullSupportSufficiencyMIPackage F :=
    FullSupportSufficiencyMIPackage_of_FaddeevEntropyForm F hfadStruct
  have hblock : FullSupportBlockMI F :=
    FullSupportBlockMI_of_FaddeevEntropyForm F hfadStruct
  have hboundary : FullSupportMIRepExtendsToBoundary F :=
    FullSupportMIRepExtendsToBoundary_of_supportRestriction F hblock
  have hpkg : SufficiencyMIPackage F := hboundary hax hfull
  obtain ⟨alpha, hα_pos, hrep⟩ := hpkg
  intro A O _ _ _ _ P q q'
  have hscaled : F.rel P q q' ↔
      alpha * mutualInfo q P ≥ alpha * mutualInfo q' P :=
    hrep P q q'
  rw [hscaled]
  constructor
  · intro hge
