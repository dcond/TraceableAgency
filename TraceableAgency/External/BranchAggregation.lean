...
signed-law tangent space.  It is a theorem (not a convention) that follows from
...
/-- Full-support finite distributions admit a positive branch mass dominated by
...
support-face identification, so they are kept as a separate convention/interface.
-/
...
/-- Explicit convention that representatives on a boundary face are chosen
...
convention, packaged under the historical name. -/
theorem boundaryValueTransport_of_supportFaceRepresentativeConvention
    (hface : FiniteSupportFaceRepresentativeConventionAssumptions.{u}) :
    FiniteBranchBoundaryValueTransportAssumptions.{u} where
  boundary_value_transport := hface.support_face_value_transport

/-- Explicit convention choosing positive boundary branch coefficients. -/
...
choice convention. -/
def boundaryFaceScale_of_coefficientScaleConvention
    (hcoeff : FiniteBoundaryCoefficientScaleConventionAssumptions.{u}) :
...
This is **not** a convention choice — it has definite mathematical content.
The positive boundary coefficient `boundaryCoeff(q,r)` is separately chosen by
`FiniteBoundaryCoefficientScaleConventionAssumptions`, but the scalar transport
...
/-- Support-face marginal-value transport convention for the HM integral
representatives.
...
theorem boundaryLinearPartTransport_of_integralRepresentation
    ...
    (htransport :
      FiniteSupportFaceMarginalValueTransportConvention.{u}
        hint hboundary) :
...
/-- Boundary summand identity from the support-face value and coefficient
...
theorem branchFormulaBoundarySummandFor_of_value_and_coefficient_transport
  ...
  boundary_summand_linearPart_eq := by
    ...
    have hvalue_transport :
      ...
      exact (hvalue.boundary_value_transport F hax hV r Q).symm
    calc
      hlin.linearPart F hV q
          (posteriorLawSignedSMul ((Channel.outcomeMarginal P₁ q) target)
            (posteriorLawDifferenceExp (branchPosterior P₁ q target)
...
/-- Singleton branch coefficient convention.
...
not behaviourally identified; the paper fixes it by positive convention so the
...
boundary full-to-face coefficient, and singleton/degenerate convention. -/
...
coefficients, boundary coefficients, or singleton conventions.  It states only
...
structure FiniteBranchAggregationFormulaAssumptions.{v}
    ...
    (hsingle : FiniteBranchSingletonScaleConventionAssumptions.{v}) where
...
structure FiniteBranchAggregationFormulaFor
    ...
    (hpath : BranchFullSupportPathIndependenceStructure F hV)
    (hboundary : FiniteBranchBoundaryFaceScaleAssumptions.{u})
    (hsingle : FiniteBranchSingletonScaleConventionAssumptions.{u}) where
  branch_aggregation_formula :
    ∀ {A O₁ O₂ : Type u} [Fintype A] [DecidableEq A] [Nonempty A]
      [Fintype O₁] [DecidableEq O₁] [Fintype O₂] [DecidableEq O₂]
      (q : Dist A) (_hq : q.FullSupport)
...
structure FiniteBranchFormulaSummandAssumptions
    ...
    (hpath : BranchPathTangentScalarStructure F hV hlin)
    (hboundary : FiniteBranchBoundaryFaceScaleAssumptions.{u})
    (hsingle : FiniteBranchSingletonScaleConventionAssumptions.{u}) : Prop where
...
theorem branchAggregationFormulaTangentFor_of_boundaryTransport
  ...
  branchAggregationFormulaTangentFor_of_boundarySummandsFor
    F hax hV hlin hpath hboundary hsingle
    (branchFormulaBoundarySummandFor_of_value_and_coefficient_transport
      hlin hboundary hvalue hcoeff F hax hV)
