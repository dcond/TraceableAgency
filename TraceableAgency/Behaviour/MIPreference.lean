...
theorem MIPrefFamily_is_MIRep : MIRep MIPrefFamily := by
...
/-- Relation transfer from an arbitrary `MIRep F` to the canonical MI family. -/
theorem rel_iff_MIPrefFamily_of_MIRep {F : PrefFamily.{u}} (hrep : MIRep F)
    {A O : Type u} [Fintype A] [DecidableEq A] [Fintype O] [DecidableEq O]
    (P : Channel A O) (q q' : Dist A) :
...
/-- Strict-relation transfer from an arbitrary `MIRep F` to the canonical MI family. -/
theorem strictRel_iff_MIPrefFamily_of_MIRep {F : PrefFamily.{u}} (hrep : MIRep F)
    {A O : Type u} [Fintype A] [DecidableEq A] [Fintype O] [DecidableEq O]
    (P : Channel A O) (q q' : Dist A) :
...
/-- Transfer for the bundled block-experiment predicate used in A2. -/
theorem experimentPairPref_iff_MIPrefFamily_of_MIRep {F : PrefFamily.{u}} (hrep : MIRep F)
...
/-!
## A1 Benchmark Component

MIPrefFamily satisfies A1 (Weak Order and Local Nontriviality).
-/
...
/-- MIPrefFamily satisfies A1 (Weak Order and Local Nontriviality). -/
theorem MIPrefFamily_A1 : A1_WeakOrderLocalNontriviality MIPrefFamily := by
  constructor
  · intro A O _ _ _ _ P
...
/-!
## A3 Benchmark Component

MIPrefFamily satisfies A3 (Block-Comparison Coherence).
-/
...
For the second part of A2, we use entropy reduction plus weak convergence against H.
-/
...
theorem MIRep_A1 {F : PrefFamily.{v}} (hrep : MIRep F) :
    A1_WeakOrderLocalNontriviality F := by
  constructor
  · intro A O _ _ _ _ P
    constructor
    · intro q q'
      rcases (MIPrefFamily_A1.1 P).1 q q' with h | h
    ...
    · intro q q' q'' hqq' hq'q''
      ...
      exact (MIPrefFamily_A1.1 P).2 q q' q''
